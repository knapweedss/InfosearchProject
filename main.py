...
def load_all():
    ...
    return data_texts, bert_ind, bert_vec, tokenizer, model, bm25_cnt_vec, bm25_ind, bm25_tfidf_vec, tfidf_res, tfidf_ind, tfidf_w, tfidf_vec
...
def return_dataframe(search_output):
    df = pd.DataFrame(search_output, columns=['Вот что думают пользователи мейл.ру:'])
    return st.dataframe(df.style.set_properties(**{'background-color': 'white'}), use_container_width=True)


def my_search_engine(data_texts, bert_ind, bert_vec, tokenizer, model, bm25_cnt_vec, bm25_ind, bm25_tfidf_vec, tfidf_res, tfidf_ind, tfidf_w, tfidf_vec):
    """
    Лэйаут поисковика и сам поиск
    """
    st.set_page_config(
    layout="wide",
...
def start_process():
    data_texts, bert_ind, bert_vec, tokenizer, model, bm25_cnt_vec, bm25_ind, bm25_tfidf_vec, tfidf_res, tfidf_ind, tfidf_w, tfidf_vec = load_all()
    my_search_engine(data_texts, bert_ind, bert_vec, tokenizer, model, bm25_cnt_vec, bm25_ind, bm25_tfidf_vec,
                     tfidf_res, tfidf_ind, tfidf_w, tfidf_vec)
